...
class RatioSampler(Sampler):
    def __init__(self, labels, attack_label = 1, attack_ratio = 0.1, seed=100, epoch_length=None):
...
class SplitSampler:
    def __init__(self, train_split: float = 0.7, test_split: float = 0.15, validation_split: float = 0.15, seed: int = 432):
        abs_split_error = abs(1 - (train_split + test_split + validation_split))
        if abs_split_error > TOLERANCE:
            raise ValueError(f"train test and validation split must equal to one, currently got: {train_split=}, {test_split=}, {validation_split=}")
    ...
    def load_file(cls, path: Path | PathLike) -> Self:
        ...
        loaded_sampler = cls(train_split=loaded['train_split'], test_split=loaded['test_split'], validation_split = loaded['validation_split'], seed = loaded['seed'])
    ...
    def build_split(self,item_ids: Iterable, labels: Iterable) -> Self:
        ...
        for label, items in by_label.items():
            for pos, item_id in items:
                h = int(hashlib.sha256(f"{self.__seed}-{label}-{item_id}".encode()).hexdigest(), 16)
    ...
    def save_split(self, path: Path | PathLike) -> None:
        ...
        torch.save({'train_split': self._train_split,
                   'test_split': self._test_split,
                   'validation_split': self._validation_split,
                   'split': self.__split, 'seed': self.__seed}, path)
